...
def get_form_details(form):
    try:
        ...
        for textarea in form.find_all("textarea"):
            ...
            inputs.append({"name": name, "type": "textarea", "value": value})
        ...
        for select in form.find_all("select"):
            ...
            inputs.append({"name": name, "type": "select", "value": value})
...
# HTML injection scanner for one URL
def scan_html_injection(url, payloads, output):
    ...
    for i, form in enumerate(forms, start=1):
        ...
        for payload in payloads:
            ...
            if response and payload in response.text:
                msg = f"[HTML INJECTION] {url} | Form #{i} | Payload: {payload}"
...
def scan_urls(urls, payloads, output):
    log_result("[HTML Injection Scan Started]\n", output)
    for url in urls:
        scan_html_injection(url, payloads, output)
