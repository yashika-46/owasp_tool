...
def submit_form(form_details, url, payload):
    try:
        ...
        for input_field in form_details['inputs']:
            if input_field["type"] in ["text", "search", "textarea", "email", "url"]:
...
def scan_js_injection(url, payloads, output=None):
...
def scan_urls(urls, payloads, output):
    print("\nStarting JavaScript Injection Scan...\n")

    for url in urls:
        scan_js_injection(url, payloads, output)

    if output:
        print(f"\nScan Complete. Log saved to: {output}")
    else:
        print("\nScan Complete. No output file specified.")
...
if __name__ == "__main__":
    ...
    if input_type == 'url':
        ...
        scan_urls([url], JS_PAYLOADS, output if output else None)

    elif input_type == 'file':
        file_path = input("Enter the file path containing URLs (one per line): ").strip()
        try:
            urls = load_file(file_path)
            scan_urls(urls, JS_PAYLOADS, output if output else None)
        except Exception as e:
